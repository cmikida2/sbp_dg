...
def main(write_output=True, order=4):
    ...
    def u_analytic(x):
        return f(-(c[0]*x[0] + c[0]*x[1])/norm_c+sym.var("t", sym.DD_SCALAR) *
                 norm_c)
    ...
    for j in range(0, n_sbp_y):
        for i in range(0, n_sbp_x):
            u_sbp[i + j*(n_sbp_x)] = np.sin(1*(-(c[0]*x_sbp[i]
                                                 + c[0]*y_sbp[j])/norm_c))
    ...
    # Initial energy
    P_full = np.kron(np.eye(n_sbp_y), dx*p_x).dot(np.kron(dy*p_y,
                                                          np.eye(n_sbp_x)))
    u_work = u_sbp.get()
    ...
    east2west, west2east, mass, Pf2g, Pg2f = sbp_sbp_projection(n_sbp_y-1,
                                                                order_sbp,
                                                                n_sbp_y-1,
                                                                order_sbp)

    gr2gl = np.kron(Pf2g, np.eye(n_sbp_x))
    mass_bold = np.kron(mass, np.eye(n_sbp_x))
    Pg2f_bold = np.kron(Pg2f, np.eye(n_sbp_x))
    Eo_gl = np.kron(np.eye((n_sbp_y-1)*order_sbp), e_l_matx)
    En_gl = np.kron(np.eye((n_sbp_y-1)*order_sbp), e_r_matx)
    Et_gl = np.kron(np.eye((n_sbp_y-1)*order_sbp), e_t_matx)
    ...
    def rhs(t, u):
        ...
        # Projection from SBP east to SBP west will be included later.
        # sbp_east = east2west.dot(sbp_east)

        # We also need the FD-to-glue description of the state.
        u_gl_ts = gr2gl.dot(u_sbp_ts)

        gl_w = Pg2f_bold.dot(Eo_gl.dot(u_gl_ts))
        gl_e = Pg2f_bold.dot(Et_gl.dot(En_gl.dot(u_gl_ts)))
        ...
        for j in range(0, n_sbp_y):
            ...
            left_bcx = sbp_east[j]
            # TWO-WAY BOUNDARY
            dl_xbc = c_l_x*((gl_w[j*n_sbp_x] - gl_e[j*n_sbp_x]) +
                            (1.0/(2.0*tau_l))*((v_l_x - gl_w[j*n_sbp_x]))) - \
                c_r_x*(1.0/(2.0*tau_l))*(left_bcx - gl_e[j*n_sbp_x])
            # ONE-WAY BOUNDARY
            # dl_xbc = c_l_x*((gl_w[j*n_sbp_x] - gl_e[j*n_sbp_x]) +
            #                 (1.0/(2.0*tau_l))*((v_l_x - gl_w[j*n_sbp_x])))
        ...
        # FINAL (PROVABLE) STEP - TWO-WAY BOUNDARY:
        rhs_energy = dy*u_gl_ts.dot(mass_bold).dot(c[0]*Eo_gl - 2*tau_l*Eo_gl +
                                                   2*tau_l*Et_gl.dot(En_gl) -
                                                   c[0]*En_gl).dot(u_gl_ts)
        # FINAL (PROVABLE) STEP - ONE-WAY BOUNDARY:
        # rhs_energy = u_sbp_ts.dot(-E_nx).dot(boldp_y).dot(u_sbp_ts) + \
        #     dy*u_gl_ts.dot(mass_bold).dot(c[0]*Eo_gl - 2*tau_l*Eo_gl +
        #                                   2*tau_l *
        #                                   Et_gl.dot(En_gl)).dot(u_gl_ts)
            ...
            for j in range(0, n_sbp_y):
                for i in range(0, n_sbp_x):
                    sbp_error[i + j*n_sbp_x] = u_sbp[i + j*n_sbp_x].get() - \
                        np.sin(1*(-(c[0]*x_sbp[i] + c[0]*y_sbp[j])/norm_c +
                                  last_t*norm_c))
