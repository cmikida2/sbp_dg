...
def test_convergence_advec(actx_factory, order, order_sbp, spacing_factor, c):
    actx = actx_factory()
    ...
    for nelem in nelems:
        ...
        # Actually use the smallest timestep we have for all runs:
        dt = dt_factor * (1.0 / 60.0) / 4**2
        # For good measure, and based on running convergence estimates
        # for the highest grid spacings,
        # multiply this by an additional factor of 1/2.
        dt = dt * 0.5
        ...
        for event in dt_stepper.run(t_end=final_time):
            if isinstance(event, dt_stepper.StateComputed):
                ...
                vis.write_vtk_file("Results/eoc_dg-%s-%04d.vtu" %
                                   (nelem, step),
                ...
                filename = "Results/eoc_sbp_%s-%04d.vts" % (nelem, step)
                import os
