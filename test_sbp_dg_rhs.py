...
@pytest.mark.parametrize("order_sbp", [2, 4, 6])
@pytest.mark.parametrize("spacing_factor", [2, 3, 4, 5])
@pytest.mark.parametrize("c", [np.array([0.27,  0.31])])
def test_rhs_convergence_advec(actx_factory, order, order_sbp, spacing_factor,
                               c, visualize=False):
    actx = actx_factory()
    ...
    for nelem in nelems:
        ...
        sbp_bdry_discr = dcoll.discr_from_dd(dof_desc.DTAG_BOUNDARY(
            "btag_sbp"))
        ...
        def fp(x):
            return actx.np.cos(10*x)

        def rhs_analytic(x, t=0):
            return 10*norm_c*fp(-c.dot(x)/norm_c+t*norm_c)
        ...
        # Call the RHS:
        rhs_comb = np.zeros(int(n_sbp_x*n_sbp_y) + nnodes)
        rhs_comb = rhs(0, u_comb)
        u_sbp = rhs_comb[0:int(n_sbp_x*n_sbp_y)]
        u_dg = unflatten(actx, dcoll.discr_from_dd("vol"),
                         actx.from_numpy(rhs_comb[int(n_sbp_x*n_sbp_y):]))
        last_t = 0

        error_l2_dg = op.norm(
            dcoll,
            u_dg - rhs_analytic(nodes, t=last_t),
            2
        )
        print('DG L2 Error ', error_l2_dg)
        ...
        if visualize:
            filename = "Results/eoc_sbp_%s-rhs.vts" % (nelem)
            import os
        ...
        # The important bit - tracking error.
        # print(h, error_l2_dg + error_l2_sbp)
        # eoc_rec.add_data_point(h, error_l2_dg + error_l2_sbp)
        eoc_rec.add_data_point(h, error_l2_dg.get())

    print("------------------------------------------------------")
    print("Advection Speed(s): %f %f" % (c[0], c[1]))
    print("SBP Global Order: %d" % (order_sbp/2 + 1))
    print("DG Order: %d" % order)
    print("SBP Spacing Factor: %d" % spacing_factor)
    print("------------------------------------------------------")
    print(eoc_rec.pretty_print(abscissa_label='h', error_label='DG L2 Error'))
    print("Order Estimate for this Case: %f" % eoc_rec.order_estimate())

    assert eoc_rec.order_estimate() > (order_sbp/2 + 0.5) * 0.95
